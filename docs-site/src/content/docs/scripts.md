...
剧本只有「私有」与「公开」两种实际状态（仅作者可设置）：

- **私有**：不对外可见
- **公开**：出现在在线剧本库，任何人可导入并订阅

> 早期的「固定快照 / 浮动最新」引用模式选择器已不再显示：精确版本回放尚未实现（读取始终跟随目标剧本的最新内容），如需锁定某个版本，请用 **Fork** 复制一份独立副本。
...
在剧本列表的行操作菜单，或详情面板顶部「更多操作」下拉里，选「公开」/「取消公开」。公开前系统会检查设定核对状态，未核对的剧本需先完成「设定核对」。
...
详情面板顶部「版本历史」按钮 → 打开右侧抽屉，列出编辑记录（提交信息、类型、作者、时间；最新的在最上面，记录多时可「加载更多」翻页）。

哪些操作会留记录：编辑章节正文、增删改世界书条目 / 时间线锚点 / 知识库实体、Fork 等，各留一条。

**章节正文的连续编辑按「编辑会话」合并为一条**：同一章持续编辑期间（默认 10 分钟内）只记一条，列表里显示「含 N 次保存」；停手一段时间后再编辑会开新记录。所以编辑器每 2.5 秒的自动保存不会堆出大量条目。

**回退**：章节类记录右侧的「恢复到此前」可用——把该章正文回退到这条记录发生之前（回退本身也会留一条记录，可再点它退回）。其余类型（世界书 / 时间线锚点 / 知识库实体 / 副本）目前没有按版本回退的端点，按钮置灰并说明原因。整本剧本级的「回滚到某个版本」尚未实现。
