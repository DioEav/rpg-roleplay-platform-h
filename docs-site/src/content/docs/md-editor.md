...
编辑区底部状态栏有一个「版本历史」按钮（打开章节标签时可用）→ 弹出该章的**改动历史**列表：

- **手动编辑**：你在编辑器里的保存（含每 2.5 秒的自动保存）留一条，标题形如「手动编辑章节 #1」；
- **AI 改写**：右栏 AI 助手改写本章后留一条，标题形如「AI 改写章节 #1」；
- **回滚 / 已撤销**：恢复或撤销操作留下的记录（自身也带快照，可再次退回）。

**连续编辑会合并**：同一章的持续编辑（默认 10 分钟内）只记一条，列表里显示「含 N 次保存」；停手一段时间后再编辑会开新记录。所以不必担心自动保存堆积记录。

每条记录右侧的「恢复到此前」可把该章正文回退到**这条记录发生之前**的版本（对 AI 写、手动写、以及「恢复 / 撤销」操作本身都可用）。**恢复本身也是可逆的**：点错了，重新点底部「版本历史」打开列表（恢复成功后抽屉会自动关闭），新产生的那条「恢复章节 #N 到版本 #M 之前」记录右侧同样有「恢复到此前」，点它即可退回恢复之前的状态。

> 恢复成功后编辑器里的正文不会立即变化（同标签内不回灌外部内容，防止冲掉你正在输入的文字与光标位置）——关闭该章节标签再打开，或切换到别章再切回来，即可看到恢复后的正文。

列表里积累了不需要的记录时，每条右侧的「删除」可移除它：**只删这条记录（及其改前快照），正文不受影响**，删除后无法恢复。

手动编辑的记录不会进入右栏 AI 助手的「撤销」按钮（那个只服务 AI 改动），两者互不干扰。

> 整本剧本级的「回滚到此版本」暂未开放，见「剧本」文档的「查看版本历史」。
